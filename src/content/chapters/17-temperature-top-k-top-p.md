...
出力分布から token を選ぶ規則。この章では、全体のどこでこの部品が必要になり、何を受け取り、何を返すかを先に固定する。

## 直感

temperature は logits の差を縮小・拡大する。top-k/top-p は候補集合を切ってから再正規化する。

## 実際の機構

ここで扱う計算を短く書くと、`p = softmax(logits / temperature)` となる。記号は小さな例で確認してから使う。教育用の図と数値は概念を追うために小さくしており、実モデルの内部状態をそのまま可視化したものではない。
...
1. 「Temperature, top-k, top-p」で、入力と出力はそれぞれ何か。
